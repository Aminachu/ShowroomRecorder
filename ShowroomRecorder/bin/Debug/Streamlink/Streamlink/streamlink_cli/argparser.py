...
def build_parser():
    ...
    player.add_argument(
        ...
        This is a shell-like syntax to support using a specific player:
          ...
          Streamlink.exe --player=vlc <url> [stream]

        Absolute or relative paths can also be passed via this option in the
        event the player's executable can not be resolved:

          Streamlink.exe --player=/path/to/vlc <url> [stream]
          Streamlink.exe --player=./vlc-player/vlc <url> [stream]

        To use a player that is located in a path with spaces you must quote the
        parameter or its value:

          Streamlink.exe "--player=/path/with spaces/vlc" <url> [stream]
          Streamlink.exe --player "C:\\path\\with spaces\\mpc-hc64.exe" <url> [stream]

        Options may also be passed to the player. For example:

          Streamlink.exe --player "vlc --file-caching=5000" <url> [stream]
    ...
    player.add_argument(
        ...
        Example:
          ...
          Streamlink.exe -p vlc -a "--play-and-exit {{filename}}" <url> [stream]
